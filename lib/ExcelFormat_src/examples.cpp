...
static void read_big_sheet(const char * path, const int row, const int col)
{
  BasicExcel xls(path);
  ...
  cout<< " raed big_sheet " << path << " " << row << "/" <<col <<" : "<< cell->GetString() << "\n";
  xls.Close(); 
}
...
//	_CrtSetDbgFlag(_CrtSetDbgFlag(0)|_CRTDBG_LEAK_CHECK_DF);
	...
	read_big_sheet("big-example.xls", 1,2);
