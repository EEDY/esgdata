...
def delete_data_from_linux(linux_data_dir, nodes):
  ...
  for node in nodes:
    for disk in linux_data_dir:
      proc = Process(target=run_linux_cmd, args=("rm -f %s/*.dat" % (disk), node, True))
      plist.append(proc)
...
def gen_data_thread(excel, dir, node, rcount, child, parallel):
...
def gen_data(excel_file, dirs, nodes, rcount, parallel):
    ...
    for node, nid in zip(nodes, range(len(nodes))):
        for i in range(thread_per_node):
            para_list[nid].append((None, {"excel": excel_file, "dir": dirs[diskid], "node": node, "rcount": rcount,
    ...
    for idx in range(thread_extra):
        para_list[0].append((None, {"excel": excel_file, "dir": dirs[0], "node": node, "rcount": rcount,
...
def check_data_dir(data_dir_list, nodes):
