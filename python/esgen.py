...
def delete_data_from_linux(nodes):
...
def gen_data_thread(excel, dir, node, rcount, child, parallel):
  ...
  cmd = ESGEN_ROOT + "/esgen -INPUT " + excel \
                   + " -RCOUNT " + str(rcount) \
                   + " -TERMINATE N " \
                   + " -DIR " + dir \
                   + " -QUIET Y " \
  ...
  info = "[Node: " + node + "][" + table + "-" + str(child) + "] generating data"
  return (fail, info)'''
...
def main():
    ...
    gen_data(ESGN_HOME + os.path.basename(options.excel), data_dir_list, nodes, options.rcount, options.parallel)
